...
// Messenger-style page with manual dark mode using JS conditional
...
export default function MessengerPage() {
  ...
  try {
    ...
    data.id = "R"+data.id
     ...
     setConversations((prev) =>
      prev.map((c) => (c.id === activeConvId ? { ...c, messages: [...c.messages, data ] } : c))
    );
  ...
  return (
    ...
    >
      {/* Sidebar */}
      <aside
        className={`w-80 border-r border-gray-700 bg-white ${dark ? "dark:bg-slate-900 dark:text-slate-100" : ""}`}
      >
        <div
          className={`p-4 border-b border-gray-700 ${dark ? "dark:bg-slate-900 dark:text-slate-100" : ""}`}
        >
      ...
      >
        ...
        >
          <div  className={`  justify-between border-0 bg-white   flex items-center gap-4 ${
            dark ? "dark:bg-slate-900 dark:text-slate-100" : ""
          }`}>
            <div
          ...
          <div >
            ...
            <div className="text-xs text-slate-500">
              {activeConv?.lastSeen} • Active
                  ...
                  <HomeIcon onClick={()=>{router.replace('/userpage')}} className="h-6 w-6 hover:text-blue-800 active:text-blue-900  cursor-pointer text-blue-600" />
        </div>
